...
class DocQR {
    ...
    public Object getQR(Map<String, Object> params) {
        ...
        CardAttachment cardAttachment = dataManager.load(CardAttachment.class)
                .query("select distinct d from wf\$CardAttachment d where d.card.id = :cardId")
                .parameter("cardId", doc.getId())
                .list();

        if(cardAttachment.signatures.size()) {
            return [['QRcodeimage1': getPDF417Barcode(cardAttachment.signatures.substring(55,100))]]

        }

        return [['QRcodeimage1': getPDF417Barcode(var)]]
