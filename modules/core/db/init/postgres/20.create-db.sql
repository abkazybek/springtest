-- begin THESISSUMMER_ORDER_LOAN
alter table THESISSUMMER_ORDER_LOAN add constraint FK_THESISSUMMER_ORDER_LOAN_ON_PROTOCOL_TEMPLATE foreign key (PROTOCOL_TEMPLATE_ID) references SYS_FILE(ID)^
alter table THESISSUMMER_ORDER_LOAN add constraint FK_THESISSUMMER_ORDER_LOAN_ON_ORDER_RECEIVER foreign key (ORDER_RECEIVER_ID) references DF_EMPLOYEE(CORRESPONDENT_ID)^
alter table THESISSUMMER_ORDER_LOAN add constraint FK_THESISSUMMER_ORDER_LOAN_ON_CARD foreign key (CARD_ID) references WF_CARD(ID) on delete CASCADE^
create index IDX_THESISSUMMER_ORDER_LOAN_ON_PROTOCOL_TEMPLATE on THESISSUMMER_ORDER_LOAN (PROTOCOL_TEMPLATE_ID)^
create index IDX_THESISSUMMER_ORDER_LOAN_ON_ORDER_RECEIVER on THESISSUMMER_ORDER_LOAN (ORDER_RECEIVER_ID)^
-- end THESISSUMMER_ORDER_LOAN
-- begin THESISSUMMER_ORDER_INS
alter table THESISSUMMER_ORDER_INS add constraint FK_THESISSUMMER_ORDER_INS_ON_PROTOCOL_TEMPLATE foreign key (PROTOCOL_TEMPLATE_ID) references SYS_FILE(ID)^
alter table THESISSUMMER_ORDER_INS add constraint FK_THESISSUMMER_ORDER_INS_ON_ORDER_RECEIVER foreign key (ORDER_RECEIVER_ID) references DF_EMPLOYEE(CORRESPONDENT_ID)^
alter table THESISSUMMER_ORDER_INS add constraint FK_THESISSUMMER_ORDER_INS_ON_CARD foreign key (CARD_ID) references WF_CARD(ID) on delete CASCADE^
create index IDX_THESISSUMMER_ORDER_INS_ON_PROTOCOL_TEMPLATE on THESISSUMMER_ORDER_INS (PROTOCOL_TEMPLATE_ID)^
create index IDX_THESISSUMMER_ORDER_INS_ON_ORDER_RECEIVER on THESISSUMMER_ORDER_INS (ORDER_RECEIVER_ID)^
-- end THESISSUMMER_ORDER_INS
-- begin THESISSUMMER_ORDER_TRANFER_MONEY
alter table THESISSUMMER_ORDER_TRANFER_MONEY add constraint FK_THESISSUMMER_ORDER_TRANFER_MONEY_ON_ORDER_RECEIVER foreign key (ORDER_RECEIVER_ID) references DF_EMPLOYEE(CORRESPONDENT_ID)^
alter table THESISSUMMER_ORDER_TRANFER_MONEY add constraint FK_THESISSUMMER_ORDER_TRANFER_MONEY_ON_CARD foreign key (CARD_ID) references WF_CARD(ID) on delete CASCADE^
create index IDX_THESISSUMMER_ORDER_TRANFER_MONEY_ON_ORDER_RECEIVER on THESISSUMMER_ORDER_TRANFER_MONEY (ORDER_RECEIVER_ID)^
-- end THESISSUMMER_ORDER_TRANFER_MONEY
-- begin THESISSUMMER_ORDER_LOAN_SI
alter table THESISSUMMER_ORDER_LOAN_SI add constraint FK_THESISSUMMER_ORDER_LOAN_SI_ON_ORDER_RECEIVER foreign key (ORDER_RECEIVER_ID) references DF_EMPLOYEE(CORRESPONDENT_ID)^
alter table THESISSUMMER_ORDER_LOAN_SI add constraint FK_THESISSUMMER_ORDER_LOAN_SI_ON_CARD foreign key (CARD_ID) references WF_CARD(ID) on delete CASCADE^
create index IDX_THESISSUMMER_ORDER_LOAN_SI_ON_ORDER_RECEIVER on THESISSUMMER_ORDER_LOAN_SI (ORDER_RECEIVER_ID)^
-- end THESISSUMMER_ORDER_LOAN_SI
-- begin THESISSUMMER_ORDER_VOZVRATV_GU
alter table THESISSUMMER_ORDER_VOZVRATV_GU add constraint FK_THESISSUMMER_ORDER_VOZVRATV_GU_ON_ORDER_RECEIVER foreign key (ORDER_RECEIVER_ID) references DF_EMPLOYEE(CORRESPONDENT_ID)^
alter table THESISSUMMER_ORDER_VOZVRATV_GU add constraint FK_THESISSUMMER_ORDER_VOZVRATV_GU_ON_CARD foreign key (CARD_ID) references WF_CARD(ID) on delete CASCADE^
create index IDX_THESISSUMMER_ORDER_VOZVRATV_GU_ON_ORDER_RECEIVER on THESISSUMMER_ORDER_VOZVRATV_GU (ORDER_RECEIVER_ID)^
-- end THESISSUMMER_ORDER_VOZVRATV_GU
-- begin THESISSUMMER_ORDER_RETURN_NEISP_SUB1C
alter table THESISSUMMER_ORDER_RETURN_NEISP_SUB1C add constraint FK_THESISSUMMER_ORDERETUNEISSUB1_ON_ORDER_RETURN_NEISP_SUB foreign key (ORDER_RETURN_NEISP_SUB_ID) references THESISSUMMER_ORDER_RETURN_NEISP_SUB(CARD_ID)^
create index IDX_THESISSUMMER_ORDERETUNEISSUB1_ON_ORDER_RETURN_NEISP_SUB on THESISSUMMER_ORDER_RETURN_NEISP_SUB1C (ORDER_RETURN_NEISP_SUB_ID)^
-- end THESISSUMMER_ORDER_RETURN_NEISP_SUB1C
-- begin THESISSUMMER_ORDER_RETURN_NEISP_SUB
alter table THESISSUMMER_ORDER_RETURN_NEISP_SUB add constraint FK_THESISSUMMER_ORDER_RETURN_NEISP_SUB_ON_ORDER_RECEIVER foreign key (ORDER_RECEIVER_ID) references DF_EMPLOYEE(CORRESPONDENT_ID)^
alter table THESISSUMMER_ORDER_RETURN_NEISP_SUB add constraint FK_THESISSUMMER_ORDER_RETURN_NEISP_SUB_ON_CARD foreign key (CARD_ID) references WF_CARD(ID) on delete CASCADE^
create index IDX_THESISSUMMER_ORDER_RETURN_NEISP_SUB_ON_ORDER_RECEIVER on THESISSUMMER_ORDER_RETURN_NEISP_SUB (ORDER_RECEIVER_ID)^
-- end THESISSUMMER_ORDER_RETURN_NEISP_SUB
-- begin THESISSUMMER_ORDER_REMBURSEMENT1C
alter table THESISSUMMER_ORDER_REMBURSEMENT1C add constraint FK_THESISSUMMER_ORDER_REMBURSEMENT1C_ON_ORDER_REMBURSEMENT foreign key (ORDER_REMBURSEMENT_ID) references THESISSUMMER_ORDER_REMBURSEMENT(CARD_ID)^
create index IDX_THESISSUMMER_ORDER_REMBURSEMENT1C_ON_ORDER_REMBURSEMENT on THESISSUMMER_ORDER_REMBURSEMENT1C (ORDER_REMBURSEMENT_ID)^
-- end THESISSUMMER_ORDER_REMBURSEMENT1C
-- begin THESISSUMMER_ORDER_REMBURSEMENT
alter table THESISSUMMER_ORDER_REMBURSEMENT add constraint FK_THESISSUMMER_ORDER_REMBURSEMENT_ON_ORDER_RECEIVER foreign key (ORDER_RECEIVER_ID) references DF_EMPLOYEE(CORRESPONDENT_ID)^
alter table THESISSUMMER_ORDER_REMBURSEMENT add constraint FK_THESISSUMMER_ORDER_REMBURSEMENT_ON_CARD foreign key (CARD_ID) references WF_CARD(ID) on delete CASCADE^
create index IDX_THESISSUMMER_ORDER_REMBURSEMENT_ON_ORDER_RECEIVER on THESISSUMMER_ORDER_REMBURSEMENT (ORDER_RECEIVER_ID)^
-- end THESISSUMMER_ORDER_REMBURSEMENT
-- begin THESISSUMMER_ORDER_POSTING1C
alter table THESISSUMMER_ORDER_POSTING1C add constraint FK_THESISSUMMER_ORDER_POSTING1C_ON_ORDER_POSTING foreign key (ORDER_POSTING_ID) references THESISSUMMER_ORDER_POSTING(CARD_ID)^
create index IDX_THESISSUMMER_ORDER_POSTING1C_ON_ORDER_POSTING on THESISSUMMER_ORDER_POSTING1C (ORDER_POSTING_ID)^
-- end THESISSUMMER_ORDER_POSTING1C
-- begin THESISSUMMER_ORDER_POSTING
alter table THESISSUMMER_ORDER_POSTING add constraint FK_THESISSUMMER_ORDER_POSTING_ON_ORDER_RECEIVER foreign key (ORDER_RECEIVER_ID) references DF_EMPLOYEE(CORRESPONDENT_ID)^
alter table THESISSUMMER_ORDER_POSTING add constraint FK_THESISSUMMER_ORDER_POSTING_ON_CARD foreign key (CARD_ID) references WF_CARD(ID) on delete CASCADE^
create index IDX_THESISSUMMER_ORDER_POSTING_ON_ORDER_RECEIVER on THESISSUMMER_ORDER_POSTING (ORDER_RECEIVER_ID)^
-- end THESISSUMMER_ORDER_POSTING
-- begin THESISSUMMER_ORDER_REVALUATION1C
alter table THESISSUMMER_ORDER_REVALUATION1C add constraint FK_THESISSUMMER_ORDER_REVALUATION1C_ON_ORDER_REVALUATION foreign key (ORDER_REVALUATION_ID) references THESISSUMMER_ORDER_REVALUATION(CARD_ID)^
create index IDX_THESISSUMMER_ORDER_REVALUATION1C_ON_ORDER_REVALUATION on THESISSUMMER_ORDER_REVALUATION1C (ORDER_REVALUATION_ID)^
-- end THESISSUMMER_ORDER_REVALUATION1C
-- begin THESISSUMMER_ORDER_REVALUATION
alter table THESISSUMMER_ORDER_REVALUATION add constraint FK_THESISSUMMER_ORDER_REVALUATION_ON_ORDER_RECEIVER foreign key (ORDER_RECEIVER_ID) references DF_EMPLOYEE(CORRESPONDENT_ID)^
alter table THESISSUMMER_ORDER_REVALUATION add constraint FK_THESISSUMMER_ORDER_REVALUATION_ON_CARD foreign key (CARD_ID) references WF_CARD(ID) on delete CASCADE^
create index IDX_THESISSUMMER_ORDER_REVALUATION_ON_ORDER_RECEIVER on THESISSUMMER_ORDER_REVALUATION (ORDER_RECEIVER_ID)^
-- end THESISSUMMER_ORDER_REVALUATION
-- begin THESISSUMMER_ORDER_TRANFER1C
alter table THESISSUMMER_ORDER_TRANFER1C add constraint FK_THESISSUMMER_ORDER_TRANFER1C_ON_ORDER_TRANFER foreign key (ORDER_TRANFER_ID) references THESISSUMMER_ORDER_TRANFER(CARD_ID)^
create index IDX_THESISSUMMER_ORDER_TRANFER1C_ON_ORDER_TRANFER on THESISSUMMER_ORDER_TRANFER1C (ORDER_TRANFER_ID)^
-- end THESISSUMMER_ORDER_TRANFER1C
-- begin THESISSUMMER_ORDER_TRANFER
alter table THESISSUMMER_ORDER_TRANFER add constraint FK_THESISSUMMER_ORDER_TRANFER_ON_ORDER_RECEIVER foreign key (ORDER_RECEIVER_ID) references DF_EMPLOYEE(CORRESPONDENT_ID)^
alter table THESISSUMMER_ORDER_TRANFER add constraint FK_THESISSUMMER_ORDER_TRANFER_ON_CARD foreign key (CARD_ID) references WF_CARD(ID) on delete CASCADE^
create index IDX_THESISSUMMER_ORDER_TRANFER_ON_ORDER_RECEIVER on THESISSUMMER_ORDER_TRANFER (ORDER_RECEIVER_ID)^
-- end THESISSUMMER_ORDER_TRANFER
-- begin THESISSUMMER_ORDER_WRITE_OFF1C
alter table THESISSUMMER_ORDER_WRITE_OFF1C add constraint FK_THESISSUMMER_ORDER_WRITE_OFF1C_ON_ORDER_WRITE_OFF foreign key (ORDER_WRITE_OFF_ID) references THESISSUMMER_ORDER_WRITE_OFF(CARD_ID)^
create index IDX_THESISSUMMER_ORDER_WRITE_OFF1C_ON_ORDER_WRITE_OFF on THESISSUMMER_ORDER_WRITE_OFF1C (ORDER_WRITE_OFF_ID)^
-- end THESISSUMMER_ORDER_WRITE_OFF1C
-- begin THESISSUMMER_ORDER_WRITE_OFF
alter table THESISSUMMER_ORDER_WRITE_OFF add constraint FK_THESISSUMMER_ORDER_WRITE_OFF_ON_ORDER_RECEIVER foreign key (ORDER_RECEIVER_ID) references DF_EMPLOYEE(CORRESPONDENT_ID)^
alter table THESISSUMMER_ORDER_WRITE_OFF add constraint FK_THESISSUMMER_ORDER_WRITE_OFF_ON_CARD foreign key (CARD_ID) references WF_CARD(ID) on delete CASCADE^
create index IDX_THESISSUMMER_ORDER_WRITE_OFF_ON_ORDER_RECEIVER on THESISSUMMER_ORDER_WRITE_OFF (ORDER_RECEIVER_ID)^
-- end THESISSUMMER_ORDER_WRITE_OFF
-- begin THESISSUMMER_ORDER_POGASH_OB
alter table THESISSUMMER_ORDER_POGASH_OB add constraint FK_THESISSUMMER_ORDER_POGASH_OB_ON_ORDER_RECEIVER foreign key (ORDER_RECEIVER_ID) references DF_EMPLOYEE(CORRESPONDENT_ID)^
alter table THESISSUMMER_ORDER_POGASH_OB add constraint FK_THESISSUMMER_ORDER_POGASH_OB_ON_CARD foreign key (CARD_ID) references WF_CARD(ID) on delete CASCADE^
create index IDX_THESISSUMMER_ORDER_POGASH_OB_ON_ORDER_RECEIVER on THESISSUMMER_ORDER_POGASH_OB (ORDER_RECEIVER_ID)^
-- end THESISSUMMER_ORDER_POGASH_OB
-- begin THESISSUMMER_ORDER_CEN_BUMAGA
alter table THESISSUMMER_ORDER_CEN_BUMAGA add constraint FK_THESISSUMMER_ORDER_CEN_BUMAGA_ON_ORDER_RECEIVER foreign key (ORDER_RECEIVER_ID) references DF_EMPLOYEE(CORRESPONDENT_ID)^
alter table THESISSUMMER_ORDER_CEN_BUMAGA add constraint FK_THESISSUMMER_ORDER_CEN_BUMAGA_ON_CARD foreign key (CARD_ID) references WF_CARD(ID) on delete CASCADE^
create index IDX_THESISSUMMER_ORDER_CEN_BUMAGA_ON_ORDER_RECEIVER on THESISSUMMER_ORDER_CEN_BUMAGA (ORDER_RECEIVER_ID)^
-- end THESISSUMMER_ORDER_CEN_BUMAGA
-- begin THESISSUMMER_ORDER_NA_VOZVRAT
alter table THESISSUMMER_ORDER_NA_VOZVRAT add constraint FK_THESISSUMMER_ORDER_NA_VOZVRAT_ON_ORDER_RECEIVER foreign key (ORDER_RECEIVER_ID) references DF_EMPLOYEE(CORRESPONDENT_ID)^
alter table THESISSUMMER_ORDER_NA_VOZVRAT add constraint FK_THESISSUMMER_ORDER_NA_VOZVRAT_ON_CARD foreign key (CARD_ID) references WF_CARD(ID) on delete CASCADE^
create index IDX_THESISSUMMER_ORDER_NA_VOZVRAT_ON_ORDER_RECEIVER on THESISSUMMER_ORDER_NA_VOZVRAT (ORDER_RECEIVER_ID)^
-- end THESISSUMMER_ORDER_NA_VOZVRAT
-- begin THESISSUMMER_ORDER_SUBSIDIY
alter table THESISSUMMER_ORDER_SUBSIDIY add constraint FK_THESISSUMMER_ORDER_SUBSIDIY_ON_ORDER_RECEIVER foreign key (ORDER_RECEIVER_ID) references DF_EMPLOYEE(CORRESPONDENT_ID)^
alter table THESISSUMMER_ORDER_SUBSIDIY add constraint FK_THESISSUMMER_ORDER_SUBSIDIY_ON_CARD foreign key (CARD_ID) references WF_CARD(ID) on delete CASCADE^
create index IDX_THESISSUMMER_ORDER_SUBSIDIY_ON_ORDER_RECEIVER on THESISSUMMER_ORDER_SUBSIDIY (ORDER_RECEIVER_ID)^
-- end THESISSUMMER_ORDER_SUBSIDIY
-- begin THESISSUMMER_PROTOCOL_KK
alter table THESISSUMMER_PROTOCOL_KK add constraint FK_THESISSUMMER_PROTOCOL_KK_ON_TYPE_PROTOCOL foreign key (TYPE_PROTOCOL_ID) references THESISSUMMER_TYPE_PROTOCOL_KK(ID)^
alter table THESISSUMMER_PROTOCOL_KK add constraint FK_THESISSUMMER_PROTOCOL_KK_ON_CITY foreign key (CITY_ID) references THESISSUMMER_CITY(ID)^
alter table THESISSUMMER_PROTOCOL_KK add constraint FK_THESISSUMMER_PROTOCOL_KK_ON_ORDER_RECEIVER foreign key (ORDER_RECEIVER_ID) references DF_EMPLOYEE(CORRESPONDENT_ID)^
alter table THESISSUMMER_PROTOCOL_KK add constraint FK_THESISSUMMER_PROTOCOL_KK_ON_JURIST foreign key (JURIST_ID) references DF_EMPLOYEE(CORRESPONDENT_ID)^
alter table THESISSUMMER_PROTOCOL_KK add constraint FK_THESISSUMMER_PROTOCOL_KK_ON_CARD foreign key (CARD_ID) references WF_CARD(ID) on delete CASCADE^
create index IDX_THESISSUMMER_PROTOCOL_KK_ON_TYPE_PROTOCOL on THESISSUMMER_PROTOCOL_KK (TYPE_PROTOCOL_ID)^
create index IDX_THESISSUMMER_PROTOCOL_KK_ON_CITY on THESISSUMMER_PROTOCOL_KK (CITY_ID)^
create index IDX_THESISSUMMER_PROTOCOL_KK_ON_ORDER_RECEIVER on THESISSUMMER_PROTOCOL_KK (ORDER_RECEIVER_ID)^
create index IDX_THESISSUMMER_PROTOCOL_KK_ON_JURIST on THESISSUMMER_PROTOCOL_KK (JURIST_ID)^
-- end THESISSUMMER_PROTOCOL_KK
-- begin THESISSUMMER_PROTOCOL_PRAV
alter table THESISSUMMER_PROTOCOL_PRAV add constraint FK_THESISSUMMER_PROTOCOL_PRAV_ON_CARD foreign key (CARD_ID) references WF_CARD(ID) on delete CASCADE^
-- end THESISSUMMER_PROTOCOL_PRAV
-- begin THESISSUMMER_SPRAVKA_PO_ZADOL
alter table THESISSUMMER_SPRAVKA_PO_ZADOL add constraint FK_THESISSUMMER_SPRAVKA_PO_ZADOL_ON_POLUCHATEL foreign key (POLUCHATEL_ID) references DF_EMPLOYEE(CORRESPONDENT_ID)^
alter table THESISSUMMER_SPRAVKA_PO_ZADOL add constraint FK_THESISSUMMER_SPRAVKA_PO_ZADOL_ON_CARD foreign key (CARD_ID) references WF_CARD(ID) on delete CASCADE^
create index IDX_THESISSUMMER_SPRAVKA_PO_ZADOL_ON_POLUCHATEL on THESISSUMMER_SPRAVKA_PO_ZADOL (POLUCHATEL_ID)^
-- end THESISSUMMER_SPRAVKA_PO_ZADOL
-- begin THESISSUMMER_ZADANIE_NA_PLATEZH
alter table THESISSUMMER_ZADANIE_NA_PLATEZH add constraint FK_THESISSUMMER_ZADANIE_NA_PLATEZH_ON_ORDER_RECEIVER foreign key (ORDER_RECEIVER_ID) references DF_EMPLOYEE(CORRESPONDENT_ID)^
alter table THESISSUMMER_ZADANIE_NA_PLATEZH add constraint FK_THESISSUMMER_ZADANIE_NA_PLATEZH_ON_CARD foreign key (CARD_ID) references WF_CARD(ID) on delete CASCADE^
create index IDX_THESISSUMMER_ZADANIE_NA_PLATEZH_ON_ORDER_RECEIVER on THESISSUMMER_ZADANIE_NA_PLATEZH (ORDER_RECEIVER_ID)^
-- end THESISSUMMER_ZADANIE_NA_PLATEZH
-- begin THESISSUMMER_PROTOCOL_FILE_TEMPLATE
alter table THESISSUMMER_PROTOCOL_FILE_TEMPLATE add constraint FK_THESISSUMMER_PROTOCOL_FILE_TEMPLATE_ON_FILE_DESCRIPTOR foreign key (FILE_DESCRIPTOR_ID) references SYS_FILE(ID)^
create index IDX_THESISSUMMER_PROTOCOL_FILE_TEMPLATE_ON_FILE_DESCRIPTOR on THESISSUMMER_PROTOCOL_FILE_TEMPLATE (FILE_DESCRIPTOR_ID)^
-- end THESISSUMMER_PROTOCOL_FILE_TEMPLATE
